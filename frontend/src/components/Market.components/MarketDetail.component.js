...
export default class MarketDetail extends Component {
  ...
  async componentDidMount() {
    try {
      // 아이템 데이터 갱신
      const result = await this.getMarketDetail(this.state.parsed.marketid);
      this.setState({ ...this.state, marketData: result });
      console.log(result);
    } catch (error) {
      console.log(error);
    }
  ...
  async getMarketDetail(marketID = 0) {
    try {
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex flex-row w-full p-5">
              ...
              <p class="flex-auto text-sm rounded-2xl m-2 p-2 border border-green-100 text-green-400 bg-green-50 text-center">
                상품 상태 {this.state.marketData.status}
              </p>
